...
def main() -> None:
    ...
    ap.add_argument("--no-enforce-quarterly-loading-constraint",
                    dest="enforce_quarterly_loading_constraint", action="store_false")
    ...
    ap.add_argument("--standardize", action="store_true", default=True,
                    help="Standardize Y inside fit_bm_dfm (default: True).")
    ...
    res = fit_bm_dfm(Y_monthly=X, y_quarterly=y, config=cfg)

    npz_path = outdir / "bm_dfm_fit.npz"
    json_path = outdir / "bm_dfm_fit.json"

    np.savez_compressed(
        npz_path,
        T=res.T, Q=res.Q, C=res.C, R=res.R,
        a0=res.a0, P0=res.P0,
        a_smooth=res.a_smooth, P_smooth=res.P_smooth, P_lag_smooth=res.P_lag_smooth,
        f_t_idx=res.f_t_idx,
