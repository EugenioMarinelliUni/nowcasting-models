...
from src.dfm_pipeline.dfm_bm_ml.fast.fit_fast import fit_bm_dfm_fast
...
def main() -> None:
    ...
    for dt in eval_idx:
        ...
        res = fit_bm_dfm_fast(Y_monthly=Y_monthly, y_quarterly=y_quarterly, config=config)

        out_path = out_dir / f"bm_dfm_fast_{dt.strftime('%Y-%m-%d')}.npz"
        np.savez_compressed(
            out_path,
