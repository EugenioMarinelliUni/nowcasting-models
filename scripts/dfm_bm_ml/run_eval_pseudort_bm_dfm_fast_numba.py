...
from dfm_pipeline.dfm_bm_ml.fast.fit_fast_numba import fit_bm_dfm_fast_numba as fit_bm_dfm_fast
...
def _load_panel(panel_csv: str, date_col: str) -> pd.DataFrame:
    ...
    if date_col not in X.columns:
        raise ValueError(f"date_col={date_col!r} not found in panel CSV columns.")
    X[date_col] = pd.to_datetime(X[date_col])
    return X.set_index(date_col).sort_index()
...
def _load_target(target_csv: str, date_col: str, target_col: str) -> pd.Series:
    ...
    if date_col not in y.columns:
        raise ValueError(f"date_col={date_col!r} not found in target CSV columns.")
    if target_col not in y.columns:
        raise ValueError(f"target_col={target_col!r} not found in target CSV columns.")
    y[date_col] = pd.to_datetime(y[date_col])
    return y.set_index(date_col).sort_index()[target_col]


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Pseudo-real-time evaluation of the BM-DFM (fast numba, econometric-spec patch).",
    ...
    parser.add_argument("--update-initial-state-each-iter", dest="update_initial_state_each_iter", action="store_true", default=True)
    parser.add_argument("--no-update-initial-state-each-iter", dest="update_initial_state_each_iter", action="store_false")
    ...
    parser.add_argument("--train-max-iter", default=None, type=int)
    ...
    X_full = X_full.loc[common_idx].apply(pd.to_numeric, errors="coerce")
    y_full = pd.to_numeric(y_full.loc[common_idx], errors="coerce")
    ...
    (outdir / "run_config.json").write_text(json.dumps(cfg_dump, indent=2), encoding="utf-8")
