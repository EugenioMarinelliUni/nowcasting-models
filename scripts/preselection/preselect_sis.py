#!/usr/bin/env python3
# Location: scripts/preselection/preselect_sis.py
...
def build_shifted_columns(X: pd.DataFrame, x_leads: int, x_lags: int) -> pd.DataFrame:
    """Return a wide DataFrame with original columns plus shifts: suffix __Fk (lead k>0) and __Lk (lag k>0)."""
    out = {}
    for c in X.columns:
        out[c] = X[c]
...
def run_sis(cfg_path: str,
    ...
    # build shifted design (train & full) and align y
    Xtr_sh = build_shifted_columns(Xtr0, x_leads=x_leads, x_lags=x_lags)
    Xtr = Xtr_sh.loc[ytr.index].dropna(how="all", axis=1)  # drop all-NaN shift columns on train
    ...
    for c in Xtr.columns:
        ...
        corrs[c] = 0.0 if denom == 0.0 else float((xm @ ym) / denom)

    s = pd.Series(corrs).abs().sort_values(ascending=False)

    # threshold / top-N
    sel = s.index.tolist()
    if tau_sis and tau_sis > 0:
        sel = s[s >= tau_sis].index.tolist()
    if top_n and top_n > 0 and len(sel) > top_n:
        sel = sel[:top_n]

    # de-duplicate, then guardrails (use train covariances of selected set)
    Xtr_sel = Xtr.loc[:, sel] if len(sel) else Xtr.iloc[:, :0]
    cols = dedup_by_corr(Xtr_sel, sel, dedup_tau)

    # top-up to min_features using rank order
    ...
    # prepare the full panel with the exact selected shifted columns
    X_full_sh = build_shifted_columns(X_full, x_leads=x_leads, x_lags=x_lags)
    X_out = X_full_sh.loc[:, cols]

    meta = write_meta(panel, tag, "sis",
                      {"tau_sis": tau_sis, "top_n": top_n,
                       "min_features": min_features, "max_features": max_features,
                       "dedup_tau": dedup_tau, "x_leads": int(x_leads), "x_lags": int(x_lags)},
                      cols)
    outp = write_panel(panel, tag, "sis", X_out, cols) if write_panel_flag else None
    msg = f"{panel} {tag} SIS: selected {len(cols)} vars; meta={meta}"
...
if __name__ == "__main__":
    ...
    ap.add_argument("--cfg", default=None, help="run one backtest config (path). Default: iterate all in config.yaml")
    ap.add_argument("--tau_sis", type=float, default=0.0)
    ...
    for c in cfgs:
        run_sis(c, monthly_freq=mf,
                tau_sis=args.tau_sis, top_n=args.top_n,
                min_obs_train=min_obs, write_panel_flag=args.write_panel,
                min_features=args.min_features, max_features=args.max_features,
                dedup_tau=args.dedup_tau, x_leads=args.x_leads, x_lags=args.x_lags)
