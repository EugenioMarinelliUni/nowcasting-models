...
from dfm_pipeline.dfm_dyn.backtest import rolling_dfm_nowcast
...
def compute_within_quarter_rmse(
    ...
    eval_start:
        ...
        Typically equal to the start-date of the nowcast evaluation (e.g. 2000-01-01).
    ...
    # Months where actual quarterly GDP (standardized) is observed
    dot_mask = df["y"].notna()
    ...
    # For each quarter (identified by its "dot" month):
    #   date_dot = first month of quarter (where y is non-NaN)
    #   h=1 -> y_hat at date_dot
    #   h=2 -> y_hat at date_dot + 1 month
    #   h=3 -> y_hat at date_dot + 2 months
...
def run_nowcast(
    ...
    # Determine start_idx from start_date
    ...
    print(f"DFM spec: r={r}, p={p}, max_iter_em={max_iter_em}, tol_em={tol_em}")

    y_hat, models = rolling_dfm_nowcast(
        X=X,
        y=y,
        r=r,
        p=p,
    ...
    # Overall evaluation: where both y and y_hat are non-NaN
    ...
    out_path = out_dir / f"dfm_nowcast_r{r}_p{p}_{label}.csv"
    ...
    print(f"Saved nowcast results to: {out_path}")

    # Within-quarter RMSEs: h=1,2,3 (first/second/third month of quarter)
    # Build a small df for evaluation
    ...
    print("Within-quarter RMSEs (standardized target):")
    for h in (1, 2, 3):
        rmse_h = rmses_h[h]
        n_h = counts_h[h]
        if np.isnan(rmse_h):
...
def main():
    parser = argparse.ArgumentParser(
        description="Run Bańbura–Modugno-style DFM rolling nowcasts on standardized panels, with within-quarter RMSE evaluation."
    )
    parser.add_argument(
        "--panel-csv",
        type=str,
    ...
    parser.add_argument(
        "--out-dir",
        type=str,
        default="results/dfm_bm_nowcasts",
