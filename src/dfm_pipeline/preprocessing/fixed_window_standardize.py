...
def standardize_panel_on_window(
    ...
    *,
    ...
    """
    Standardize (z-score) a stationarized panel using μ,σ computed on a single training window.
    Returns (Z_train, stats). Z_train spans exactly [start, end].
    """
    X = X_raw.copy()
    ...
    stats = FixedStdStats(mean=mu, std=sd,
